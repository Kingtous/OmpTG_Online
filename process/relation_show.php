...
<div class="agile-its">
    ...
    <div class="w3layouts">
        <p>Generate the Relation file and ALF code for every OpenMP task based on ALF code.</p>
...
//                            showMessage("Path: " . $_FILES["file"]["tmp_name"]);
                            ...
                            if (create_temp_folder_for_current_id()){
                                ...
                                if (move_uploaded_file($path,$file)){
                                    $command = WCTG.' -i' . $file . " -t $pPath/relation &2>1";
                                    my_exec(session_id(),$command);
                                    showMessage("See relation folder in files for more details.");
                                    show_home_btn();
                                }
                                else{
                                    showMessage(IOError);
