...
<html>
<head>
    <title>OmpTG--PCFG Generator</title>
...
<div class="agile-its">
    <h2>File Upload</h2>
    <div class="w3layouts">
        <p>Convert ALF Code into Parrallel Control Flow Graph</p>
                    ...
                    if (isset($_POST['start_process'])) {
                        try{
                            ...
                            if (file_exists($pPath.'/'.$dotFile) and file_exists($pPath.'/'.$wctFile) and file_exists($pPath.'/'.$relFile) ){
                                $command = command_gen_pcfg($pPath.'/'.$dotFile,$pPath.'/'.$wctFile,$pPath.'/'.$relFile);
                                my_exec(session_id(),$command);
                                // 检查是否生成图
                                $graphFile = $pPath.'/'.'FinalOutput.pdf';
                                if (file_exists($graphFile)){
                                    rename_and_show_download_btn(null,$graphFile);
                                }
                                show_home_btn();
                            }
                            else{
...
//                                echo '<br>'.'移动成功'.'</br>';
                                    ...
                                    if (file_exists($folder) || mkdir($folder)) {
                                        ...
                                        if ($dot_file == null or $wct_file == null or $rel_file == null) {
                                            ...
                                            //显示结果
                                            echo '<form id="file_select_form" method="post" action="pcfg_show.php">';
                                            //显示
