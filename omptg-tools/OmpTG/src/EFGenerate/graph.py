...
def changeShapeOfCondition(graph):
    ...
    for node in nx.nodes(graph):
        try:
            ...
            html_output('Warning: ' + node + ' ERROR in Function:' + 'changeShapeOfCondition')
            continue
        ...
        for line in text:
            ...
            if line != '':
                ...
                try:
                    ...
                    continue
        if DEBUG:
            html_output('WCET Dict=', end='')
            html_output(WCET_Varience)

    except:
        html_output('WCET FILE ERROR')
    finally:
        file.close()
...
def parrallel(graph):
    ...
    for node in nx.nodes(graph):
        try:
            ...
            html_output('Warning: ' + node + ' ERROR in Function:' + 'parrallel')
            continue
...
def printFeatureOfGraph(graph):
    ...
    # 输出--Terminal
    if DEBUG:
        html_output('========Debug Message Start=========')
        html_output('Vertex(|V|): ' + str(Nodes))
        html_output('Edges(|E|): ' + str(Edges))
        html_output('Call_TaskFunc(N_ce): ' + str(Call_TaskFunc))
        html_output('Wait Vertex(N_we): ' + str(Wait_Vertex))
        html_output('Condition Vertex(N_cd): ' + str(ConditionVertex))
        html_output('AverageConditionalBranch(N_br): ' + str(AverageConditionBranch))
        html_output('Average WCET(C): ' + str(AverageWCET))
        html_output('WCET_Varience(e): ' + str(WCET_Varience))
        html_output('-----------Extra Message------------')
        html_output('TotalConditionBranch:', TotalConditionBranch)
        html_output('WCET_Total', WCET_Total)
        html_output('========Debug Message End=========')
    # 输出--文件
    ...
    try:
        ...
        html_output('I/O Error.')
    finally:
        file.close()


def pdfhtml_output(Path):
    import os
    ...
    for node in graph.node:
        try:
            ...
            if graph.node[node]['shape'] == 'diamond':
                ...
                if DEBUG:
                    html_output(node, graph.node[node]['shape'], count)
            else:
                continue
        except:
            continue
...
def calcBranch(graph):
    ...
    for gh in result:
        ...
        for node in gh.node:
            ...
            if count != 0:
                numset.add(count)
        html_output("Debug: " + entryNode.replace('_entry', ' numset:'), max(numset))
        if max(numset) > 1:
            # >1 表示有条件分支
...
def genEFG(graph, path, keylist, relation):
    ...
    # 输出DOT文件
    html_output('输出第'+str(output_cnt)+'张图')
...
def graphEFG(graph, relation):
    ...
    # html_output(ModeDict['diff'].pathModel.pathList)
    html_output('路径搜索完成，开始生成图')
...
def init_argv(Argslist,current_dir):
    global dotPath,relationPath,wctPath,root,dotOutput,EFGDir,parseFunction
...
if __name__ == '__main__':
    ...
    op_p = False  # parseFunction
    op_d = False  # dot
    op_r = False  # relation
    op_w = False  # wct
    # Process
    getArgs = {}
    options, args = getopt.getopt(sys.argv[1:], "d:r:w:p:")
    ...
    if (op_d == False) or (op_r == False) or (op_w == False) or (op_p == False):
        html_output('Please Import the file u\'d like to ')

        # //html_output(current_dir1)
    else:
        init_argv(getArgs, currentdir)
        html_output("处理Relation表...")
        relation = parseRelation(relationPath)
        # 预处理
        html_output("预处理CFG...")
        preprocess(dotPath)
        # 得到cluster定义
        html_output("获取图的Cluster__定义...")
